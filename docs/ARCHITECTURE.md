...
The normal asset uses `ipa` and `g2lex-espeak-ipa-v1`. The Piper asset uses `espeak-ipa3` and `g2lex-espeak-piper-ipa3-v1`. Both use one initialized eSpeak-NG backend, voice, binary/data fingerprints, ordered generated keys, and shared union/generated inventory provenance. The source union remains complete provenance, while both outputs contain the same deterministic pronounceable subset. Keys with empty output in both modes are recorded in `skipped_keys`; asymmetric mode availability is rejected, and generated values are never empty. The Piper value is preserved exactly, including U+200D, and is suitable for `[[ ... ]]` raw phoneme injection. Unsupported voices are explicit coverage omissions rather than fallbacks.
